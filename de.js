...
const fs = require('fs')
const res = fs.readFileSync('de.in').toString('ascii')
outp=decrypt(Buffer.from(res,'base64')).toString().split("&rid=")[1].split("\x00")[0]
fs.writeFile('de.out', outp, function(err) { 
    if(err) { 
